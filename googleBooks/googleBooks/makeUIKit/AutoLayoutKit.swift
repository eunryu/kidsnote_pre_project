...
open class AutoLayoutKit {
    ...
    // 기준점보다 크거나 작다 에 대한 표현 추가 - 2024.02.22
    // X 보다 크다
    open func setWidthNotEqualSize(targetView: AnyObject, standardSize: CGFloat, isBig: Bool) {
        if isBig {
            // 기준보다 커야한다
            targetView.addConstraint(NSLayoutConstraint(item: targetView, attribute: .width, relatedBy: .greaterThanOrEqual, toItem: nil, attribute: .notAnAttribute, multiplier: 1.0, constant: standardSize))
        } else {
            // 기준보다 작아야 한다
            targetView.addConstraint(NSLayoutConstraint(item: targetView, attribute: .width, relatedBy: .lessThanOrEqual, toItem: nil, attribute: .notAnAttribute, multiplier: 1.0, constant: standardSize))
        }
    }
    
    open func setHeightNotEqualSize(targetView: AnyObject, standardSize: CGFloat, isBig: Bool) {
        if isBig {
            // 기준보다 커야한다
            targetView.addConstraint(NSLayoutConstraint(item: targetView, attribute: .height, relatedBy: .greaterThanOrEqual, toItem: nil, attribute: .notAnAttribute, multiplier: 1.0, constant: standardSize))
        } else {
            // 기준보다 작아야 한다
            targetView.addConstraint(NSLayoutConstraint(item: targetView, attribute: .height, relatedBy: .lessThanOrEqual, toItem: nil, attribute: .notAnAttribute, multiplier: 1.0, constant: standardSize))
        }
    }
    
    // 다른 뷰랑 왼쪽 동일하게 적용
    open func equalViewLeading(targetView: AnyObject, standardView: AnyObject, mainView: AnyObject) {
        mainView.addConstraint(NSLayoutConstraint(item: targetView, attribute: .leading, relatedBy: .equal, toItem: standardView, attribute: .leading, multiplier: 1.0, constant: 0))
    }
    
    // 다른 뷰랑 왼쪽 동일하게 적용
    open func equalViewTrailing(targetView: AnyObject, standardView: AnyObject, mainView: AnyObject) {
        mainView.addConstraint(NSLayoutConstraint(item: targetView, attribute: .trailing, relatedBy: .equal, toItem: standardView, attribute: .trailing, multiplier: 1.0, constant: 0))
    }
    
    open func equalViewTop(targetView: AnyObject, standardView: AnyObject, mainView: AnyObject) {
    ...
    open func equalViewLeadTrail(targetView: AnyObject, standardView: AnyObject, mainView: AnyObject) {
        ...
        self.equalViewTrailing(targetView: targetView, standardView: standardView, mainView: mainView)
    }
}
