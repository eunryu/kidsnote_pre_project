...
class DetailCreatorInfoCell: BaseTableViewCell {
    ...
    override func initView() {
        ...
        dateInfoLabel = MakeUILabelKit.shared.makeLabel("-", size: CGSize(width: 100, height: 25), addView: self)
    ...
    func initData(bookInfo: GoogleBookInfo) {
        // date 변환
        let convert = ToolManager.shared.convertStringDateFormat(preType: .apiDate, aftType: .showDate, origin: bookInfo.publishedDate)
        dateInfoLabel.text = "\(convert), \(bookInfo.publisher)"
    }
}
